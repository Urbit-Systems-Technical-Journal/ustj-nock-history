...
\newcommand{\affiliation}{Zorp Corp, Urbit Foundation}
...
Source:  \patp{sorreg-namtyv} \& \patp{fodwyt-ragful} (2018). %\citet{Nock4K}

\section{The Future of Nock}

While deviations from the trunk line of the Nock family have been proposed at various points,\footnote{Notably, Ax (see \patp{barpub-tarber}'s Ax, pp.~191–227 in this volume), \textsc{skew}, and \textsc{plan} (see \texttt{@sol\_plunder}, pp.~227–XXX in this volume).} Nock itself has remained the definitional substrate of Urbit since its inception.  It has also been adopted as the primary \textsc{isa} of Nockchain and the NockApp ecosystem.
...
The most recent version, Nock 4K, appears to provide all of the opcodes necessary for correct and efficient evaluation.\footnote{Modulo the vagaries of the von Neumann architecture, etc.  See also the critique of \texttt{@sol\_plunder}, pp. XXX--XXX in this volume.}  It is likely that future versions of Nock will be based genetically on Nock 4K, but with some changes to improve its performance and usability.  The road to zero kelvin is likely very long still, given an abundance of caution, but it also appears to be straight.
