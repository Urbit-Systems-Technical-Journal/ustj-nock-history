...
\newcommand{\authorname}{N. E. Davis~\patp{lagrev-nocfep},\\Curtis Yarvin~\patp{sorreg-namtyv}}
\newcommand{\affiliation}{Zorp Corp, Urban Tiger}
...
\fancyfoot[L]{{\footnotesize
            ...
            %   Manuscript submitted for review.\\
              {\it Urbit Systems Technical Journal} II:1 (2025):  155–190. \\
              ~ \\
...
\sloppy
Nock is a combinator calculus which serves as the computational specification layer for the Urbit and Nockchain/NockApp systems.  It is a hyper-\textsc{risc} instruction set architecture (ISA) intended for execution by a virtual machine (but see \patp{mopfel-winrux}, pp.~99–130 in this volume).  Nock's simplicity and unity of expression make it amenable to proof-based reasoning and guarantees of correctness.  Its Lisp-like nature surfaces the ability to introspect on the code itself, a property which higher-level languages compiling to it can exploit.  Yet for all this, Nock was not born from a purely mathematical approach, but found its roots in practical systems engineering.
...
\begin{itemize}
  ...
  \item  Functional (as in language).  Nock is a pure function of its arguments.  In practice, the Urbit operating system provides a simulated global scope for userspace applications, but this virtualized environment reduces to garden-variety Nock.  (See \patp{lacnes}, pp.~71–97 in this volume, for details of a Nock virtualized interpreter.)
...
Source:  \patp{sorreg-namtyv} (2008d). %\citet{Nock13K}
...
Source:  \patp{sorreg-namtyv} (2008c). %\citet{Nock12K}
...
Source:  \patp{sorreg-namtyv} (2008b). %\citet{Nock11K}
...
Source:  \patp{sorreg-namtyv} (2008a). %\citet{Nock10K}
...
Source:  \patp{sorreg-namtyv} (2010c). %\citet{Nock9K}
...
Source:  \patp{sorreg-namtyv} (2010b). %\citet{Nock8K}
...
Source:  \patp{sorreg-namtyv} (2010a). %\citet{Nock7K}
...
Source:  \patp{sorreg-namtyv} (2011). %\citet{Nock6K}
...
Source:  \patp{sorreg-namtyv}, \patp{rovnys-ricfer}, \& \patp{fodwyt-ragful} (2018). %\citet{Nock4K}

\section{The Future of Nock}

While deviations from the trunk line of the Nock family have been proposed at various points,\footnote{Notably, Ax (see \patp{barpub-tarber}'s Ax, pp.~191–228 in this volume), \textsc{skew}, and \textsc{plan}.} Nock itself has remained the definitional substrate of Urbit since its inception.  It has also been adopted as the primary \textsc{isa} of the NockApp ecosystem.

Why, then, do we contemplate further changes?  The \textsc{skew} proposal spearheaded by \patp{little-ponnys} and \patp{siprel} (2020) argued that Nock 4K represented an undesirable saddle point in the design space of possible Nocks, itself a ``ball of mud''.  While \textsc{skew} itself was not adopted, it inspired the development of Plunder and \textsc{plan} as a solid-state computing architecture sharing some ambitions with Urbit and Nock \citep{Plunder2025}.  A rigorously æsthetic argument can thus be sustained that Nock is not yet ``close enough'' to its final, diamond-perfect form to be a viable candidate.
